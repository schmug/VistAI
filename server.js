// Ultra-simple search engine server without dependencies 
// This is a ES module version that will work in Replit's environment
...
const server = http.createServer(async (req, res) => {
  ...
  if (path === '/' || path === '/index.html') {
    try {
      const htmlContent = await readFile(join(__dirname, 'index.html'), 'utf8');
  ...
  // Handle API track click
  if (path === '/api/track-click' && req.method === 'POST') {
    let body = '';
    ...
    req.on('end', async () => {
      try {
        ...
        if (!resultId || typeof resultId !== 'number') {
          res.writeHead(400, { 'Content-Type': 'application/json' });
          return res.end(JSON.stringify({ error: 'Invalid resultId' }));
        }
